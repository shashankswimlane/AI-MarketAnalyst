...
// Turn a raw tool invocation into a human-readable status like
// "Creating Button.jsx" (in progress) or "Edited App.jsx" (done).
function getToolStatusLabel(tool: {
  toolName: string;
  args?: any;
  state?: string;
}): string {
  ...
  const fileName = (p?: string) =>
    p ? p.split("/").filter(Boolean).pop() || p : "file";

  if (tool.toolName === "str_replace_editor") {
    switch (args.command) {
      case "create":
        return `${done ? "Created" : "Creating"} ${fileName(args.path)}`;
      case "str_replace":
      case "insert":
        return `${done ? "Edited" : "Editing"} ${fileName(args.path)}`;
      case "view":
        return `${done ? "Viewed" : "Viewing"} ${fileName(args.path)}`;
    }
  }
  ...
  if (tool.toolName === "file_manager") {
    switch (args.command) {
      case "rename":
        return `${done ? "Renamed" : "Renaming"} ${fileName(args.path)}`;
      case "delete":
        return `${done ? "Deleted" : "Deleting"} ${fileName(args.path)}`;
    }
  }
                          ...
                          case "tool-invocation":
                            ...
                            return (
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs border border-neutral-200">
                                ...
                                <span className="text-neutral-700">{getToolStatusLabel(tool)}</span>
                              </div>
                            );
                          case "source":
                            return (
